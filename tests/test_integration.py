...
class TestIntegration:
    ...
    @responses.activate
    @patch('lambda_function.suumo_scraper.extract_address_from_suumo')
    ...
    def test_full_workflow_address_input(self, mock_format, mock_get_hazard, mock_scraper):
        # モックの設定
    ...
    @patch('lambda_function.hazard_info.get_all_hazard_info')
    @patch('lambda_function.hazard_info.format_all_hazard_info_for_display')
    def test_full_workflow_suumo_url(self, mock_format, mock_get_hazard, mock_scraper):
        # モックの設定
        mock_scraper.return_value = "東京都新宿区西新宿1-1-1"
        mock_get_hazard.return_value = {'flood': 'high'}
        mock_format.return_value = {'洪水': '高リスク'}
            ...
            "events": [
                {
                    ...
                    "message": {
                        "type": "text",
                        "text": "https://suumo.jp/chintai/tokyo/example"
                    },
                    "replyToken": "test_reply_token"
                }
            ]
